...
def trips_csv(request):
    ...
    writer.writerow(['ID', 'Chauffør Navn', 'Bil Nr.', 'Fra(D)', 'Vl Nr.', 'Fra(T)', 'Til(D)', 'Til(T)', 'Betaling', 'Cash',
                    'Total', 'Aftaletype', 'Procent%', '%Indkomst', 'Timer', 'Pause', 'Betalte Timer', 'Udgifter', 'Udgifter Grund', 'Note'])
    # Loop through and output
    for trip in trips:
        writer.writerow([trip.id, trip.driver, trip.car, trip.fromd, trip.vl, trip.fromt, trip.tilld, trip.tillt, trip.income, trip.cash, trip.total,
                        trip.p_type, trip.percent, trip.per_total, trip.work_time, trip.pause, trip.p_time, trip.expenses, trip.exp_reason, trip.note])
